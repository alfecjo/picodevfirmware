...
static float read_bmp280_temperature(const char unit)
{
    int32_t temp_raw;

    bmp280_read_raw_temp(&temp_raw);
    float temp_c = bmp280_convert_temp_float(temp_raw, &params) / 100.0f;
...
static void publish_temperature(MQTT_CLIENT_DATA_T *state)
{
    ...
    const char *temperature_key = full_topic(state, "school/room1/temperature");

    float temperature = read_bmp280_temperature(TEMPERATURE_UNITS);
